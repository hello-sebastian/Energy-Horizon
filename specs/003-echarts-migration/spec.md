...
- **FR-003a (kontekst czasu „teraz”)**: Indeks osi X dla markera „dziś” (w tym `markLine`, `markPoint`, odcinek delty, kotwica prognozy, wyróżnienie etykiety osi X w trybie adaptacyjnym) MUSI wskazywać **slot `fullTimeline`, który zawiera bieżący instant** (`Date.now()`): dla `i < N-1` przedział `[fullTimeline[i], fullTimeline[i+1])`, dla ostatniego slotu `[fullTimeline[N-1], +∞)`. Implementacja referencyjna: `findTimelineSlotContainingInstant` w `src/card/axis/now-marker-slot.ts`. Nie wolno wyznaczać tego indeksu przez `indexOf` na znaczniku północy doby wyłącznie w strefie przeglądarki ani przez wymaganie, aby `fullTimeline` zawierało dokładnie znacznik początku „dzisiejszego” dnia — przy agregacji godzinowej, tygodniowej lub miesięcznej prowadzi to do braku markera lub błędnej pozycji (np. x=0 przy widoku dobowym z slotami godzinowymi).
...
- **FR-006**: Prognoza MUSI być renderowana jako oddzielna seria liniowa z `lineStyle.type: 'dashed'`, bez wypełnienia, od indeksu slotu zawierającego bieżący instant (patrz FR-003a) do ostatniego slotu `fullTimeline`.
...
- **FR-011**: Legenda MUSI być sterowana konfiguracją karty: pole YAML `show_legend` (mapowane na `ChartRendererConfig.showLegend`) — włączona **tylko** gdy wartość jest ścisłym `true` (`=== true`). W przeciwnym razie legenda jest ukryta. Gdy jest widoczna, wyświetla etykiety serii (bieżąca, referencyjna, prognoza jeśli włączona) w kolejności czytelnej dla użytkownika; renderer MUSI ustawiać `legend.data` tak, aby ta kolejność **nie** zależała od kolejności wpisów w `series` (por. FR-020 i FR-018 w [001-time-windows-engine](../001-time-windows-engine/spec.md)).
- **FR-012**: Animacje MUSZĄ być wyłączone (`animation: false`).
- **FR-013**: Kolor z `primary_color` (lub domyślnie token karty `--eh-series-current` / `#119894` z Figmy; opcjonalnie aliasy `ha-accent` / `ha-primary-accent` / `ha-primary` albo `var(--accent-color)` itd. na hoście karty) MUSI być stosowany do: linii serii bieżącej, wypełnienia pod nią, markera „dziś" (linia przerywana + kropka bieżąca), linii prognozy.
...
- **FR-020**: Kolejność wpisów w `option.series` MUSI realizować zasadę z **FR-018** w [001-time-windows-engine](../001-time-windows-engine/spec.md): starsze warstwy pierwsze (w tym kontekst od najstarszego okna), okno bieżące nad referencją, prognoza na wierzchu gdy włączona.
...
- Tokeny CSS motywów Home Assistant (`--primary-text-color`, `--divider-color` itd.) są dostępne na hoście karty (`.ebc-card` / `ha-card`) — wykres nie wymaga osobnej palety kolorów w YAML poza opcjonalnym `primary_color` dla serii bieżącej (albo nadpisaniem `--eh-series-current` przez Card Mod).
